...
{
  esas_table <- esas_table %>% dplyr::filter(DistanceBins %in% c("0|50|100|200|300"),
                                      PlatformClass %in% c(30),
                                      Area > 0)

  observations_select_fly <- esas_table %>%
    dplyr::filter(SpeciesCode %in% species_selection,
  ...
  esas_densities_corr <- esas_table %>%
    dplyr::distinct(PositionID, Date, Time, Area, Distance, TransectWidth, Latitude, Longitude, SamplingMethod, TargetTaxa)
